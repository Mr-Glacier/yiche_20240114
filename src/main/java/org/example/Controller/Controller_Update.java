...
import org.example.Dao.Dao_Update;
...
//            System.out.println(resultJson);
    ...
    public void Method_1_Down_brand(String savePath, String main_url, String fileName) {
        String connect = Method_DownHTML(main_url);
        if (!connect.equals("Error")) {
            saveUntil.Method_SaveFile(savePath + fileName, connect);
            System.out.println("下载完成  Method_1");
        }
    }

    public void Method_2_Analysis_brand(String savePath, String fileName) {
        Dao_Update dao_brand = new Dao_Update(0, 0);
...
//        System.out.println(content);
        ...
        for (int i = 0; i < Item_zimu.size(); i++) {
            ...
            for (int j = 0; j < Item_Brands.size(); j++) {
                ...
                if (dao_brand.Method_FindExist("C_brand_id", C_brand_id)) {
                    Object obj = dao_brand.Method_Find_oneBean("C_brand_id", C_brand_id);
                    String Str1 = obj.toString();
                    if (Str1.equals(bean_brand.toString())) {
                        System.out.println("基础更新\t"+C_brand_id);
                        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                        String time = df.format(new Date());
                        bean_brand.setC_备注(time + "基础更新;");
                        bean_brand.setC_备注时间(time);
                        dao_brand.Method_Update(bean_brand, "C_brand_id", bean_brand.getC_brand_id());
                    } else {
                        System.out.println("特殊更新\t"+C_brand_id);
                        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                        String time = df.format(new Date());
                        String earlybeizhu = ((Bean_brand) obj).getC_备注();
                        bean_brand.setC_备注(earlybeizhu + time + "特殊更新;");
                        bean_brand.setC_备注时间(time);
                        dao_brand.Method_Update(bean_brand, "C_brand_id", bean_brand.getC_brand_id());
                    }
                } else {
                    System.out.println("新增品牌\t" + C_brand_id);
                    bean_brand.setC_备注("新增");
                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                    String time = df.format(new Date());
                    bean_brand.setC_备注时间(time);
    ...
    public void Method_4_Analysis_model(String savePath, String fileName) {
        Dao_Update dao_brand = new Dao_Update(0, 0);
        Dao_Update dao_model = new Dao_Update(0, 1);
        ArrayList<Object> BeanList = dao_brand.Method_Find();
...
//                            System.out.println(brand_id);
                            ...
                            if (dao_model.Method_FindExist("C_model_id", bean_model.getC_model_id())) {
                                Object obj = dao_model.Method_Find_oneBean("C_model_id", bean_model.getC_model_id());
                                if (obj.toString().equals(bean_model.toString().replace("null", "-"))) {
                                    System.out.println("基础更新\t" + bean_model.getC_model_id());
                                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                    String time = df.format(new Date());
//                                    String earlybeizhu = ((Bean_model) obj).getC_备注();
                                    bean_model.setC_备注(time + "基础更新;");
                                    bean_model.setC_备注时间(time);
                                    dao_model.Method_Update(bean_model, "C_model_id", bean_model.getC_model_id());
                                } else {
                                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                    String time = df.format(new Date());
                                    String earlybeizhu = ((Bean_model) obj).getC_备注() == null ? "" : ((Bean_model) obj).getC_备注();
                                    bean_model.setC_备注(earlybeizhu + time + "特殊更新;");
                                    bean_model.setC_备注时间(time);
                                    dao_model.Method_Update(bean_model, "C_model_id", bean_model.getC_model_id());
                                }
                            } else {
                                System.out.println("新增数据");
                                bean_model.setC_备注("新增");
                                SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                String time = df.format(new Date());
                                bean_model.setC_备注时间(time);
...
//                        saveUntil.Method_SaveFile_True("F:\\ZKZD\\Java项目\\yiche_20240114\\src\\main\\java\\org\\example\\1.txt", model_id + "\n");
    ...
    public void Method_6_Analysis_version(String savePath, String fileName) {
        Dao_Update dao_model = new Dao_Update(0, 1);
        Dao_Update dao_version = new Dao_Update(0, 2);
        ArrayList<Object> BeanList = dao_model.Method_Find();
                        ...
                        if (powerList_notSale.size() != 0) {
                            for (int j = 0; j < powerList_notSale.size(); j++) {
                                ...
                                for (int k = 0; k < carList.size(); k++) {
                                    ...
                                    if (dao_version.Method_FindExist("C_version_id",C_id)){
                                        Object obj = dao_version.Method_Find_oneBean("C_version_id", C_id);
                                        if (obj.toString().equals(bean_version.toString().replace("null", "-"))) {
                                            System.out.println("基础更新\t" + C_id);
                                            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                            String time = df.format(new Date());
                                            bean_version.setC_备注(time + "基础更新;");
                                            bean_version.setC_备注时间(time);
                                            dao_version.Method_Update(bean_version, "C_version_id",C_id);
                                        } else {
                                            System.out.println("特殊更新\t"+C_id);
                                            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                            String time = df.format(new Date());
                                            String earlybeizhu = ((Bean_version) obj).getC_备注() == null ? "" : ((Bean_version) obj).getC_备注();
                                            bean_version.setC_备注(earlybeizhu + time + "特殊更新;");
                                            bean_version.setC_备注时间(time);
                                            dao_version.Method_Update(bean_version, "C_version_id", C_id);
                                        }
                                    }else {
                                        System.out.println("新增版本\t"+C_id);
                                        bean_version.setC_备注("新增");
                                        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                                        String time = df.format(new Date());
                                        bean_version.setC_备注时间(time);
...
//                            System.out.println(onSaleCarList);
    ...
    public void Method_7_Down_Config(String savePath, String mainUrl, String fileName) {
        ...
        for (int i = 1; i <= All_GroupNumber; i++) {
            ...
            if (BeanList_FenZu.size() != 0) {
                ...
                if (DownState.equals("否")) {
                    ...
                    String connect = Method_RequestAPI(mainUrl, param);
                    if (!connect.equals("Error")) {
                        String status = JSON.parseObject(connect).getString("status");
                        if (status.equals("1")) {
...
//                1.构建空的BeanList
                    ...
                    for (int i = 0; i < bean_version_configs_carList.size(); i++) {
                       ...
                       if (dao_config.Method_FindExist("C_version_id", version_exits_id)){
                           ...
                           System.out.println("新的版本配置\t"+version_exits_id);
                           SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");//设置日期格式
                           String time = df.format(new Date());
                           bean_version_configs_carList.get(i).setC_备注(time+"新增");
